 ...
 * rather than risking an unbounded loop. A prior version with looser caps
 * (3 / 4, matching the original design) drove up to ~75 sequential model
 * calls in one browser tab worst case and reliably crashed both the
 * installed PWA and the browser tab on desktop and mobile. The caps here
 * are intentionally tighter, and `lib/useMindChain.ts` pairs this with
 * resetChat() between every call, throttled UI updates, and a yield between
 * calls — real reruns, but bounded and with the per-call overhead that was
 * also contributing to the crashes cleaned up.
 */
...
// Deliberately tighter than the original design spec (3 / 4): a full
// pipeline rerun redoes ~15 module calls, and the original caps drove up to
// ~75 sequential calls in one browser tab worst case — enough sustained
// WebGPU/WASM load to crash both the installed PWA and the browser tab, on
// desktop and mobile. These caps keep reruns real but bounded; paired with
// resetChat()-between-calls and throttled UI updates in useMindChain.ts.
