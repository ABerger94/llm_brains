...
import { ArrowLeft, Loader2, MessageSquare, RefreshCw, Target } from 'lucide-react';
import { Button } from '../components/ui';
import { GoalItem } from '../lib/data';
import { useMindStorageRefresh } from '../lib/mindStorageEvents';
import { cn } from '../lib/utils';
import { effectiveGoalRootId, goalAnswerBody, goalThreadItems, truncateDialogueSnippet } from '../lib/dialogueTranscript';
...
export default function DialogueGoalThreadPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const seed = await GoalItem.retrieve(goalId);
      if (!seed) {
        setThread([]);
        setRootId('');
        setSeedMissing(true);
      ...
      const rid = effectiveGoalRootId(seed);
      setRootId(rid);
      const all = await GoalItem.list('-created_date', LIST_CAP);
      setThread(goalThreadItems(rid, all));
    } catch (e) {
  ...
  const hrefDetail = `/dialogue/goal/${enc}`;

  if (!goalId) {
    return (
      <PageShell icon={Target} title="Thread" description={null} actions={null}>
  ...
  return (
    <PageShell
      ...
      description={
        rootId
          ? `Related goals in this pursuit cluster (root ${rootId.length > 20 ? `${rootId.slice(0, 8)}…` : rootId}).`
          : 'Related goals in this pursuit cluster.'
      }
      actions={
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {thread.map((g) => {
            ...
            const ans = truncateDialogueSnippet(goalAnswerBody(g), 360);
            const st = String(g.status || 'open');
            const oid = encodeURIComponent(String(g.id));
            return (
              <div key={g.id} className="rounded-xl border border-border bg-card p-4 shadow-sm">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-[10px] font-semibold uppercase text-amber-700 dark:text-amber-400">
                    Depth {Number(g.pursuit_depth ?? 0)}
                  </span>
                  <span
                    className={cn(
                  ...
                  {g.created_date ? (
                    <span className="text-[10px] text-muted-foreground">{moment(g.created_date).format('lll')}</span>
                  ) : null}
                </div>
                <p className="mt-2 text-sm font-medium leading-snug text-foreground">{stmt}</p>
                {ans ? <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">{ans}</p> : null}
                {String(g.id) !== goalId ? (
                  <Link
                    to={`/dialogue/goal/${oid}`}
                    className="mt-3 inline-block text-xs font-medium text-primary underline-offset-2 hover:underline"
                  >
                    Open this goal
                  </Link>
                ) : (
                  <p className="mt-3 text-[11px] text-muted-foreground">Current goal</p>
                )}
              </div>
            );
          })}
