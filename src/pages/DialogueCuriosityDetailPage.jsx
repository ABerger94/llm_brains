...
import { ArrowLeft, GitBranch, HelpCircle, Loader2, MessageSquare, RefreshCw, Users } from 'lucide-react';
import { Button } from '../components/ui';
import { CuriosityItem } from '../lib/data';
import { useMindStorageRefresh } from '../lib/mindStorageEvents';
import { cn } from '../lib/utils';
import { curiosityAnswerBody } from '../lib/dialogueTranscript';
...
export default function DialogueCuriosityDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const row = await CuriosityItem.retrieve(curiosityId);
      setItem(row || null);
    } catch (e) {
  ...
  const hrefThread = `/dialogue/curiosity/${enc}/thread`;
  const hrefCuriosity = `/curiosity?focus=${enc}`;

  const answer = item ? curiosityAnswerBody(item) : '';
  const answerDisplay = answer.length > ANSWER_MAX ? `${answer.slice(0, ANSWER_MAX)}…` : answer;

  if (!curiosityId) {
    return (
      <PageShell icon={HelpCircle} title="Question" description={null} actions={null}>
  ...
  return (
    <PageShell
      icon={HelpCircle}
      title="Question"
      description="Full text and answer from your curiosity queue."
      actions={
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
            <div className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Question</div>
