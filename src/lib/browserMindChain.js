/**
 * The 22-module cognitive pipeline used by the "Browser Mind" page — ported
 * from the llm_brains ("Mind Chain") sibling project. Runs entirely
 * client-side via WebGPU (see browserLlmEngine.js), independently of this
 * app's main server-driven pipeline (shared/pipelineModules.mjs, 11 GWT
 * modules) — the two are deliberately kept separate rather than merged into
 * one schema, since the server pipeline's prompts assume a much larger
 * SHARED_MEMORY_JSON/mind-policy runtime this client-only mode doesn't have.
 *
 * Each module is mapped to a real brain-region equivalent and one small,
 * focused call to a locally-running LLM. A module only ever sees the original
 ...
 * context window. Two modules exert real control over the run:
 *
 *  - Contradiction Engine (14) can flag up to CONTRADICTION_ENGINE_MAX_DIRECTIVES
 *    earlier modules (order < 14) for targeted re-execution via
 *    `MODULE_RERUN: [Name] — reason` lines.
 *  - Metacognition (15) can send modules 1-14 back for a fresh pass, up to
 *    METACOGNITION_MAX_RERUNS times, by starting its output with `RERUN`.
 *
 ...
 * rather than risking an unbounded loop. Caps are intentionally tight —
 * a prior looser design (3/4) drove up to ~75 sequential model calls in one
 * browser tab worst case and reliably crashed both the installed PWA and the
 * browser tab. useBrowserMindChain.js pairs this with resetChat() between
 * every call, throttled UI updates, and a yield between calls.
 */
...
export function sanitizeStageText(text) {
  return text
    .replace(/\*\*(.+?)\*\*/g, '$1')
    .replace(/(?<!\*)\*([^*\n]+?)\*(?!\*)/g, '$1')
    .replace(/^#{1,6}\s+/gm, '')
...
function depBlock(ctx, deps, labels) {
  return deps
    .map((id) => `${labels[id] ?? id}:\n${ctx.deps[id] ?? '(none yet)'}`)
    .join('\n\n');
}

function priorNoteBlock(ctx) {
...
/** ids of modules 1-13 — what Contradiction Engine can target and what a Metacognition RERUN redoes. */
...
const MIND_CHAIN_DEFS = [
  ...
  },
  {
    ...
    blurb: 'Logical consistency auditor — can flag up to 2 earlier modules for targeted re-execution.',
  ...
  },
  {
    ...
    phase: 'Audit & Control',
    title: 'Metacognition',
    blurb: 'Executive control network — can send modules 1-14 back for a fresh pass, up to once.',
...
export const MIND_CHAIN = MIND_CHAIN_DEFS.map((stage) => ({
  ...stage,
...
export function parseModuleRerunDirectives(text) {
...
export function parseMetacognitionVerdict(text) {
